...
enum CreditCardAmounts: Identifiable, Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        if let value = try? value.decode(String.self, forKey: .previousMonth){
            self = .previousMonth(amount: value)
            return
        }
        
        if let value = try? value.decode(String.self, forKey: .currentMonth){
            self = .currentMonth(amount: value)
            return
        }
        
        if let value = try? value.decode(String.self, forKey: .nextMonth){
